...
{
    public class outleetmodel_mysql_db:outleetmodel_pattern_DB
    {
        public outleetmodel_mysql_db(MySqlConnection mySqlConnection,string table_name= "outleet"):base(mySqlConnection,new MySqlCommand(),table_name)
        {
        ...
        protected override
              void InitTable(DbCommand dbCommand, string tableName)
        {
            dbCommand.CommandText= $"CREATE TABLE IF NOT EXISTS {table_name} (" +
                $"ID INT NOT NULL AUTO_INCREMENT," +
                $"storey INT UNSIGNED NOT NULL ," +
                $"area INT NOT NULL ," +
                $"PresenceOfAirConditioning TINYINT UNSIGNED NOT NULL ," +
